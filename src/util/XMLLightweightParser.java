...
public class XMLLightweightParser {
	...
	// Status used when robot is inside the xml and it looking for the tag
	// conclusion
	...
	// Number of chars that machs with the head tag. If the tailCount is equal
	// to
	...
	static {
		// Set default max buffer size to 1MB. If limit is reached then close
		// connection
	...
	public void read(IoBuffer byteBuffer) throws Exception {
		invalidateBuffer();
		// Check that the buffer is not bigger than 1 Megabyte. For security
		// reasons
		...
		// Verify if the last received byte is an incomplete double byte
		// character
		char lastChar = buf[readByte - 1];
		if (lastChar >= 0xfff0) {
			// Rewind the position one place so the last byte stays in the
			// buffer
			// The missing byte should arrive in the next iteration. Once we
			// have both
		...
		for (int i = 0; i < readByte; i++) {
			...
			if (ch < 0x20 && ch != 0x9 && ch != 0xA && ch != 0xD && ch != 0x0) {
				// Unicode characters in the range 0x0000-0x001F other than 9,
				// A, and D are not allowed in XML
				// We need to allow the NULL character, however, for Flash
				// XMLSocket clients to work.
			...
			if (isHighSurrogate) {
				if (Character.isLowSurrogate(ch)) {
					...
					// Trigger error. Found high surrogate not followed by low
					// surrogate
					throw new Exception(
				...
				// Trigger error. Found low surrogate char without a preceding
				// high surrogate
				throw new Exception(
			...
			if (status == XMLLightweightParser.TAIL) {
				...
				if (depth < 1 && ch == head.charAt(tailCount)) {
					...
					if (tailCount == head.length()) {
						// Close stanza found!
						// Calculate the correct start,end position of the
				...
				if (ch == '"') {
					...
					if (insideRootTag
						...
						int end = buffer.length() - readByte + (i + 1);
						// Skip LF, CR and other "weird" characters that could
						// appear
